...
const WOOF = getTokenBySymbol('WOOF')
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      fromCoin: WOOF as TokenInfo | null,
  ...
  mounted() {
    ...
    window.particlesJS('particles-js', {
      particles: {
        ...
        line_linked: { enable: false, distance: 150, color: '#212121', opacity: 0.4, width: 1 },
        move: {
          enable: true,
          speed: 1,
          direction: 'none',
  ...
  methods: {
    ...
    placeOrder() {
      ...
      } else if (this.endpoint === 'Woof Pool' && this.ammId) {
        const poolInfo = Object.values(this.$accessor.liquidity.infos).find((p: any) => p.ammId === this.ammId)
        swap(
          this.$web3,
          // @ts-ignore
